...
For the second phase of analysis, we will then create an aggregate model from the two initial models that is computed as a composite. Using all three models, we will conduct nested $\chi^2$-tests of likelihood to determine whether the offensive model or defensive model provides the stronger signal within the composite.
...
Since the deviance drop for offense is lower than defense, this implies the offensive model fits tighter and feeds more signal to the full model.
...
[![DOI](img/util/stargazer-R_summary-blue.png)](https://CRAN.R-project.org/package=stargazer)
