...
to identify which contributes more to winning games. According to the
research's findings, we strongly conclude that offensive and defensive
playstyles have changed over the years to shape the game into an
offense-first technical game, bringing the game even moreso towards the
non-contact and skill side of the sports spectrum.
...
three models, we will conduct nested \(\chi^2\)-tests of likelihood to
determine whether the offensive model or defensive model provides the
stronger signal within the composite.
...
Naturally, we expect to see that offensive metrics have stronger
associations to winning. While the data does seem to corroborate this
intuition, especially when a Spearman test is applied, some of the data
visually doesn't look strongly associated despite the Spearman test
saying otherwise.
...
Spearman tests confirm extremely significant results indicating that
several of these offensive metrics have not only a strong association
with winning ability, but also a positive one at that. Naturally so, but
a good reminder to have due to the perception of offense-only, defensive
...
As a result of these strong findings, we can move forward with analysis
on logistic regression fits for both offensive and defensive models, and
then further check which contributes more to winningness.

\newpage

\subsection{Analysis}\label{analysis}

Firstly, we develop an offensive model via logistic regression against
winningness. Note that during logistic regression, the range will
produce \(\text{W} \in \text{ }[0,1]\), but it is incorrect to interpret
a select value of \(\text{W}\) as a true probability of winning, as this
is not a predictive model and is only inferential. Let us describe this
full model as such

\[\mathscr O(X)=\mathbb{E}[Y|X]\] \[ \mathscr O : Y=f(X)+ \epsilon\]
...
\begin{table}[!htbp] \centering 
  \caption{Offensive Logistic Regression Results} 
  \label{} 
...
\hline \\[-1.8ex] 
 ...
 \hline \\[-1.8ex] 
Observations & 28,446 \\ 
Log Likelihood & $-$15,224.370 \\ 
Akaike Inf. Crit. & 30,468.750 \\ 
\hline 
\hline \\[-1.8ex] 
...
Since all metrics are statistically significant, there is no backwards
elimination needed.

Next, we develop an defensive model in a very similar way. Let us
describe this full model as such

\[\mathscr D(X)=\mathbb{E}[Y|X]\] \[ \mathscr D : Y=f(X)+ \epsilon\]

\[ P(Y=1|X) = \frac{1}{1+e^{-z}} \]

\[  z= \beta_0 \text{ST} + \beta_1 \text{DRB} + \beta_2 \text{PF} + \beta_3 \text{BLK}\]

After running the regression, the full defensive model produces these
results.

\begin{table}[!htbp] \centering 
  \caption{Defensive Logistic Regression Results} 
  \label{} 
\begin{tabular}{@{\extracolsep{5pt}}lc} 
\\[-1.8ex]\hline 
\hline \\[-1.8ex] 
...
\hline \\[-1.8ex] 
 ...
 \hline \\[-1.8ex] 
Observations & 28,446 \\ 
Log Likelihood & $-$16,888.260 \\ 
Akaike Inf. Crit. & 33,786.520 \\ 
\hline 
\hline \\[-1.8ex] 
\textit{Note:}  & \multicolumn{1}{r}{$^{*}$p$<$0.1; $^{**}$p$<$0.05; $^{***}$p$<$0.01} \\ 
\end{tabular} 
...
Once more there are no predictors to prune, so we leave the full
defensive model as is.

Finally, we can produce a full model \(\mathscr F\) such that

\[ \mathscr F = \mathscr O + \mathscr D \]

In similar fashion we produce and regress \(\mathscr F\), yielding the
following results.

\begin{table}[!htbp] \centering 
  \caption{Full Logistic Regression Results} 
  \label{} 
\begin{tabular}{@{\extracolsep{5pt}}lc} 
\\[-1.8ex]\hline 
\hline \\[-1.8ex] 
...
\hline \\[-1.8ex] 
 ST & $-$4.716$^{***}$ \\ 
  ...
  Assists & $-$0.022$^{***}$ \\ 
  & (0.004) \\ 
  `Offensive Rebounds` & 0.161$^{***}$ \\ 
  & (0.005) \\ 
  Turnover & $-$0.185$^{***}$ \\ 
  & (0.005) \\ 
  `Personal Fouls Drawn` & 0.151$^{***}$ \\ 
  & (0.004) \\ 
  `Blocks Against` & $-$0.099$^{***}$ \\ 
  & (0.007) \\ 
  `Defensive Rebounds` & 0.283$^{***}$ \\ 
  & (0.004) \\ 
  Steals & 0.291$^{***}$ \\ 
  & (0.007) \\ 
  `Personal Fouls` & $-$0.116$^{***}$ \\ 
  & (0.004) \\ 
  Blocks & 0.122$^{***}$ \\ 
  & (0.007) \\ 
  Constant & $-$27.913$^{***}$ \\ 
 ...
 \hline \\[-1.8ex] 
Observations & 28,446 \\ 
Log Likelihood & $-$10,808.510 \\ 
Akaike Inf. Crit. & 21,645.010 \\ 
\hline 
\hline \\[-1.8ex] 
\textit{Note:}  & \multicolumn{1}{r}{$^{*}$p$<$0.1; $^{**}$p$<$0.05; $^{***}$p$<$0.01} \\ 
\end{tabular} 
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{img/plots/corr_heatmap.png}
\vspace{0.25em}
\textbf{Figure 2.1: Correlation Heatmap}
\end{figure}

To verify that there is no colinearity, we can refer to 2.1 and identify
hot cells corresponding to metrics in the model.

We only observe a seemingly-strong colinearity between 2PP and ORB, so
we verify using a Pearson test.

\begin{figure}[H]
\centering
\includegraphics[width=1\linewidth]{img/plots/Two_Pointer_Accuracy_vs_Offensive_Rebounds.png}
\vspace{0.75em}
$\rho = -0.31, \text{ } p < 2.2 \times 10^{-16} $ \\
\textbf{Figure 2.2: Correlation Heatmap}
\end{figure}

While the p value indicates high significance of colinearity, we must
keep in mind that the the sample size is \(n = 28446\), and therefore
paired with the low \(\rho\) value we ignore any such issues.

Finally we can execute a nested likelihood-\(\chi^2\) test to see which
of the smaller model contributes more to the full composite model. Here
are the results
...
Since the deviance drop for offense is lower than defense, this implies
the offensive model fits tighter and feeds more signal to the full
model.

\subsection{Conclusions}\label{conclusions}

We can conclusively say that the stereotype is more or less right ---
offense contributes significantly more to winning games. Both defense
and offense significantly contribute to a winning game, but that finding
is more or less trivial. The difference from the ANOVA tables strongly
indicates that offense clearly has the stronger contribution between the
two.

\subsection{Citations}\label{citations}

Hlavac M (2022). stargazer: Well-Formatted Regression and Summary
Statistics Tables. Social Policy Institute, Bratislava, Slovakia. R
package version 5.2.3,
\url{https://CRAN.R-project.org/package=stargazer}.
\href{https://CRAN.R-project.org/package=stargazer}{\pandocbounded{\includegraphics[keepaspectratio]{img/util/stargazer-R_summary-blue.png}}}
